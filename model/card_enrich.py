...
uses (checked against it: research/queries/done/card_field_semantics.py):
  ...
  career_runs       its earlier rows: the table counts runs before this one
...
def enrich_card(card: pd.DataFrame, history: pd.DataFrame) -> pd.DataFrame:
    ...
    if "track" in card.columns:
        aw_card = card.get("going_description", pd.Series(index=card.index, dtype=object)).map(is_aw_going)
        tk_card = card["track"].map(_norm)
        tk_hist = hist["track"].map(_norm) if "track" in hist.columns else pd.Series(index=hist.index, dtype=object)
        aw_hist = hist.get("going_description", pd.Series(index=hist.index, dtype=object)).map(is_aw_going)
    ...
    if "horse_name" in card.columns and "horse_name" in hist.columns:
        hk_card, hk_hist = card["horse_name"].map(_norm), hist["horse_name"].map(_norm)
        for col in ("stallion", "dam_stallion", "horse_sex"):
            _fill(card, col, hk_card.map(_last_by(hist, hk_hist, col)))
        ...
        _fill(card, "career_runs", hk_card.map(hk_hist.value_counts()).fillna(0).astype(float))
    ...
    if "jockey_name" in card.columns:
        ...
        jk_hist = hist["jockey_name"].map(_norm) if "jockey_name" in hist.columns else None
        last = (_last_by(hist, jk_hist, "jockeys_claim") if jk_hist is not None else pd.Series(dtype=object))
        from_history = pd.to_numeric(card["jockey_name"].map(_norm).map(last), errors="coerce").fillna(0.0)
    ...
    if "official_rating" in card.columns:
        ...
        _fill(card, "median_or", med.where(med > 0))
    return card
