"""Connection windows and collateral form: does the 06:00 path build them as training does? The matrix against the card.

The next model's candidate blocks (iteration 67: connection windows, -0.0060
beyond form lines on the screen; iteration 66: collateral form, -0.0015),
checked before a model reading them is trained: the 944's parity query with
connection_windows and collateral among the blocks. The 944 stays the
candidate priced on both feature sets, a control that should repeat run
36230342697 (blind 0.0000; filled 0.031). What follows is that query's
docstring.

The 944: does the 06:00 path build what it was trained on? The matrix against the card, every block it reads.

The overnight run of this query (research/queries/done/matrix_live_parity.py, run
36156133812) found the live path builds the 853's and the 859's drop-in blocks
exactly as training did. The 944 (train-bfsp run 37) also reads form lines and
the form variants directly, and leaves out sire_runners and damsire_runners
(the one mismatch found then, an engine defect). The same comparison, with
form_lines and form_variants among the blocks and the 944 as the candidate.
The rest of this docstring is the original's.
...
The matrix and each of the four live builds run in a process of their own:
together in one they ran the runner out of memory (run 36155202838). Read-only.
The matrix is the one research-query.yml restores; on a miss this reports that
and stops.
"""
import gc
import io
...
CANDIDATES = [("944", 36225615528, "bfsp-model-37")]      # (label, train-bfsp run id, its model artifact)
...
OUT = Path("out/matrix_live_parity_cw")
T0 = time.time()
