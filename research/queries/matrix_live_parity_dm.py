"""The debut market block at 06:00: does the live path build the dm_ features exactly as training did?

Iteration 80 found the debut market block (model/blocks/debut_market.py: how the
market has priced each yard's debutants, second-time-outers and lightly raced
runners) worth -0.0032 beside the 958's features at the served recipe, -0.0067 in
maiden, novice and bumper races. Before a model reading it can serve, the 06:00
path must give it what training gave it: the block reads each runner's
career_runs, which on a card comes from the fill, and the market's view of the
yard's earlier runners, which must be days before today only.

Two March days (the busiest Saturday and Wednesday), the 06:00 card simulated
from the day's rows as the parity queries before it did (the card's cells, the
horse tooltip's sire, dam and sex, then the fill: the path that serves), against
training's matrix with the blocks built on it; for the 958 (run 39), the 968 (the
958 + debut_market, run 41) and its Huber twin (run 42, when it is ready), feature
by feature and in price. The pedigree market and career-before blocks, under test,
are compared feature by feature as well. Read-only.
"""
import gc
import io
import json
...
BLOCKS = ["race_relative", "time_figure", "race_relative_wide", "exposure", "head_to_head", "bookings",
          "form_lines", "form_variants", "connection_windows", "debut_market", "pedigree_market", "career_before"]
CANDIDATES = [("958", 36238560513, "bfsp-model-39"), ("968", 36258769930, "bfsp-model-41"),
              ("968h", 36264476683, "bfsp-model-42")]
...
OUT = Path("out/matrix_live_parity_dm")
VARIANTS = ("filled",)
T0 = time.time()
